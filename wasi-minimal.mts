  ...
  constructor(
    ...
    this.exports = {
      ...
      args_sizes_get: (argc_out: number, argv_buf_size_out: number) => {
        ...
        view.setUint32(
          argv_buf_size_out,
          args.reduce(function (acc: number, arg: string) {
      ...
      clock_res_get: (id: number, resolution_out: number) => {
        const view = new DataView(this.memory!.buffer);
        switch (id) {
      ...
      clock_time_get: (id: number, precision: number, time_out: number) => {
        const view = new DataView(this.memory!.buffer);
        switch (id) {
          case CLOCKID_REALTIME:
      ...
      fd_fdstat_get: (fd: number, stat_out: number) => {
        const entry = this.fds[fd];
        if (!entry) {
          return ERRNO_BADF;
        }
      ...
      fd_write: (
        ...
        console.log("FD_WRITE", fd, iovs_ptr, iovs_len, nwritten_out);
        const entry = this.fds[fd];
        if (!entry) {
          return ERRNO_BADF;
        }
